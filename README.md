...
- `colleges.json` — the 50 largest four-year US colleges by undergraduate enrollment: `id`, `name`, optional `short_name`, `city`, `state`, `undergraduates` (rounded to the nearest thousand) and `data_year`.
- `colleges.html` — a sortable, searchable table of `colleges.json`, filterable by state.
